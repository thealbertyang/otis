                ...
                <div class="panel panel-bordered">
                    <div class="panel-body users-panel">
                        <table id="dataTable" class="table table-hover">
                            ...
                            <tbody>
                            <?php  $user = \Auth::user(); $userRole = $user->role_id;  ?>
                            <?php $__currentLoopData = $dataTypeContent; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $data): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?> 
                                <?php if($data->role->id !== 1 || $userRole == 1): ?>
                                <tr>
                                    <td><?php echo e(ucwords($data->name)); ?></td>
                                    <td class="mobile-hide"><?php echo e($data->email); ?></td>
                                    <td class="mobile-hide"><?php echo e(\Carbon\Carbon::parse($data->created_at)->format('F jS, Y h:i A')); ?></td> 
                                    <td>
                                        <img src="<?php if( strpos($data->avatar, 'http://') === false && strpos($data->avatar, 'https://') === false): ?><?php echo e(Voyager::image( $data->avatar )); ?><?php else: ?><?php echo e($data->avatar); ?><?php endif; ?>" style="width:100px" class="avatar">
                                    </td>
                                    <td><?php echo e($data->role ? $data->role->display_name : ''); ?></td>
                                    <?php $trainingClass = array_values(\App\TrainingClass::all()->where('id', '=', $data->training_class_id)->toArray()); ?>
                                    <?php if(isset($trainingClass[0])): ?>
                                    <td><?php echo e($trainingClass[0]['title']); ?></td>
                                    <?php else: ?>
                                    <td>--</td>
                                    <?php endif; ?>
